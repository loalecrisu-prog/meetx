...
import 'package:meetx/screens/bottomScreen/profile/edit_profile_screen.dart';
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Profile', style: TextStyleClass.interBold(size: 20.0, color: ColorConst.black09)),
      ...
      body: ListView(
        ...
        children: [
          ...
          _NavTile(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PayScreen())),
          ),
          _NavTile(
...
class _Header extends StatelessWidget {
  const _Header({required this.name, required this.bio, required this.photoUrl, required this.onEdit});
  final String name;
  final String bio;
...
class _SliderTileState extends State<_SliderTile> {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Slider(value: value, min: widget.min, max: widget.max, onChanged: (v) => setState(() => value = v)),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
    );
  }
}
